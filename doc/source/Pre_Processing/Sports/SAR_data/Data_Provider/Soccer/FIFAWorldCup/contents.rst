FIFAWorldCup
==============
.. class:: SAR_data(data_provider='fifawc', state_def=state_def, data_path=fifawc_path, match_id=match_id, config_path=config_path, preprocess_method="SAR").preprocess_data()

    Load and process event and tracking data from CSV files and convert it into a Jsonl file.
    ...
    **Details**
    ...
    1. Convert raw FIFAWorldCup data to play.csv, ball.csv, player.csv, players.csv.
    2. Loads the event data from the CSV file specified by  `data_path/event_data`.
    3. Checks and renames the event columns to standardized names.
    4. Gets the changed player list separated by the team.
    5. Loads the player data from the CSV file specified by `data_path/player_path`.
    6. Checks and renames the player columns.
    7. Loads the home and away team tracking data from the CSV files specified by `data_path/home_tracking_path` and `data_path/away_tracking_path`.
    8. Splits the tracking data into player and ball tracking data.
    9. Checks and renames the player and ball tracking columns.
    10. Interpolates the ball tracking data.
    11. Merges the player and ball tracking data.
    12. Interpolates the player tracking data.
    13. Calculates the player and ball velocity and acceleration.
    14. Merges the event data with the tracking data.
    15. Saves the processed data to a CSV file.
